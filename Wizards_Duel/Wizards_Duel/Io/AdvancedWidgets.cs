...
namespace WizardsDuel.Io {
	...
	public class AnimationDefinition {
		...
		/// <summary>
		/// Gets or sets a value indicating whether this animation should make the event dispatcher wait for its completion
		/// before giving the next initiative.
		/// </summary>
		/// <value><c>true</c> if this instance is blocking; otherwise, <c>false</c>.</value>
	...
	public class OutObject : Icon, IComparable {
		...
		public int CurrentAnimationFrame{ get; internal set; }
			...
			set {
				if (this.facing != value) {
					...
					if (this.Facing == Facing.LEFT) {
						...
						this.animations.TryGetValue (this.CurrentAnimation, out animation);
						try {
							this.facingOffset = new Vector2f (this.Width - animation.frames[this.CurrentAnimationFrame].offset.X * 2, 0f);
						}
						catch {
							this.facingOffset = new Vector2f (this.Width - animation.frames[0].offset.X * 2, 0f);
						}
					}
					else {
		...
		virtual public bool IsAnimating {
			get {
				return (this.IsInIdle) ? false : this.animations [this.CurrentAnimation].IsBlocking;
			}
		}
